...
int setup(){
    ...
    char quat_table_file[256], reduced_data_id_file[256], start_prob_dir[256] ;
    char bg_file[256], r2peak_file[256], start_phi_file[256] ;
    int i, j, k, d, r, t, num, f_id, idx, idx_start, rank_max ;
    ...
    most_probable_num_p = malloc(s_num_data * sizeof(int)) ;
    most_probable_quat_id = malloc(s_num_data * sizeof(int *)) ;
    sprintf(infile, "%s/reduced-high_p-%d.dat", start_prob_dir, myid) ;
    fp = fopen(infile, "r") ;
    fscanf(fp, "%d", &num) ;
    for (d = 0 ; d < num ; d++){
        fscanf(fp, "%d", &most_probable_num_p[d]) ;
        most_probable_quat_id[d] = malloc(most_probable_num_p[d] * sizeof(int)) ;
        for (i = 0 ; i < most_probable_num_p[d] ; i++)
            fscanf(fp, "%d %lf", &most_probable_quat_id[d][i], &val) ;
    }
    fclose(fp) ;
    ...
    for (i = 0 ; i < iter_data_block ; i++){
        ...
        for (d = idx_start ; d < idx_start + data_block_sz ; d++){
            if (d + dmin > dmax-1)
                break ;
            for (j = 0 ; j < most_probable_num_p[d] ; j++){
                r = most_probable_quat_id[d][j] ;
                for (k = 0 ; k < num_quat_neighbor[r] ; k++)
                    if_visit_r[quat_table_idx[r][k]] = 1 ;
            }
        }
    ...
    tmplen = ((long long) max_r_block_sz)*max_num_r2hkl*3 ;
    if (tmplen > int_max){
        if (myid == 0)
            printf("max_r_block_sz*max_num_r2hkl*3 overflows int32!!\n") ;
        return 0 ;
    }
    ...
    len_d2r = calloc(s_num_data, sizeof(int)) ;
    ...
    for (d = 0 ; d < s_num_data ; d++){
        if (d + dmin > dmax-1)
            break ;
        for (j = 0 ; j < most_probable_num_p[d] ; j++){
            i = most_probable_quat_id[d][j] ;
            len_d2r[d] += num_quat_neighbor[i] ;
        }
...
void free_mem(){
    ...
    f_id = 0 ;
    for (d = dmin ; d < dmax ; d++){
        free(most_probable_quat_id[f_id]) ;
        free(ave_bg[f_id]) ;
        free(log_bg[f_id]) ;
        f_id += 1 ;
    }
    free(most_probable_quat_id) ;
    free(most_probable_num_p) ;
    free(ave_bg) ;
    free(log_bg) ;
