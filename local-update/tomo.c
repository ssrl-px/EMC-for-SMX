...
void calprob( int data_block_id ){
    ...
    int x0, y0, z0, vox_ct, row_min, col_min, row_max, col_max, photon_count ;
    int num_r2hkl_rid, d2r_table_ct_d, tomo_len, r_block_sz, indices[3] ;
    int pix_ct, high_p_count, tmplen, *idx_d2r_d, *high_p_idx, *r_block_id ;
    int **d2r_table, *d2r_table_d, *r2hkl_mpi_rid, *box_vox_id_hkl ;
    double fx, fy, fz, cx, cy, cz, vx, vy, vz, dx, dy, dz, dr2, wval, w, bv ;
    double phi_d, logP_offset_d, rescale, pval, max_p, p_sum, inv_p_sum ;
    double *ave_bg_d, *log_bg_d, *intens1_hkl, *pval_d2r_d, *high_p_val ;
    double gw2 = gw*gw, high_p_sum, rot_pix[3], rvec[3], epsilon = 1.e-10 ;
    short *data_frame_d ;
    float *pix_pid ;
    ...
    for (d = idx_start ; d < idx_start + data_block_sz ; d++){
        ...
        for (j = 0 ; j < most_probable_num_p[d] ; j++){
            ...
            for (k = 0 ; k < num_quat_neighbor[i] ; k++){
                r = rid_map[quat_table_idx[i][k]] ;
                if_visit_r[r] = 1 ;
            }
        }
        ...
        idx = d % data_block_sz ;
        d2r_table[idx] = malloc(d2r_table_ct_d * sizeof(int)) ;
        d2r_table_ct_d = 0 ;
...
void compress( int hkl_id ){
    ...
    int i, j, k, r, t, m, x, y, z, tx, ty, tz, rid, pid, indices[3] ;
    int x0, y0, z0, cur_num_hkl2r, pix_ct, vox_ct, idx, idx_offset ;
    int row_min, col_min, row_max, col_max, total_count, *box_vox_id_hkl ;
